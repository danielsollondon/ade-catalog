...
resource "azurerm_role_assignment" "st_role_admin_sbdo" {
  scope                = azurerm_storage_account.adl_st[0].id
...
resource "azurerm_storage_data_lake_gen2_filesystem" "st_adls" {
  ...
  depends_on = [
    azurerm_role_assignment.st_role_admin_sbdo,
...
module "st_pe_blob" {
  source                         = "git::https://github.com/Azure/azure-data-labs-modules.git//terraform/private-endpoint?ref=v1.5.0&depth=1"
